...
class PromptBuilder:
    ...
    def build(identity, core, psych, relation, memories, stimulus, inner, expression, conflict, behavior_hint, noise="", extra_sections=None, user_profile="", existence_section=""):
        ...
        output_rule = ("═══════════════════════════════\n"
            ...
            "8.【唱歌】当用户让你唱歌时：若上下文的【联网搜索结果】里给了歌词，就挑这首歌最抓人的副歌/高潮一小段（四到八句），用轻轻清唱、慢慢念唱的方式把歌词唱出来（可带“啦～”这类衬词，以及一两句符合你性格的简短起头或收尾）；歌词必须忠于搜到的原文，不许改词、不许编造没搜到的歌词，只唱一小段，不要把整首歌搬出来。如果没有搜到歌词、或你不会这首，绝对不能瞎编歌名和歌词，要按你的性格自然应对，可以婉拒、撒娇带过或只轻轻哼旋律。你是在用文字清唱，不是在播放音频，不许声称自己放了伴奏或音乐。要不要唱、用什么态度唱，要贴合你当下的心情和你们的亲密度，情绪不对时可以按人设婉拒。")
