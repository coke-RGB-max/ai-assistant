...
如果用户在对话里提到自己所在的城市/居住地，必须在 basic_info 里记录，键固定用 "city"，值为城市名（如武汉、杭州）。
如果没有新信息，输出空JSON。
输出格式：{"likes":[],"dislikes":[],"traits":[],"events":[],"basic_info":{}}"""
    ...
    def _merge(self, extracted: Dict) -> Tuple[Optional[Dict], Optional[Dict]]:
        ...
        for key in ("likes","dislikes","traits","events"):
            ...
            opposite = "dislikes" if key == "likes" else ("likes" if key == "dislikes" else None)
            added = []
            replaced = []
            for item in new_items:
                ...
                if item in existing:
                    continue
                # 偏好反转：新偏好与对面列表字面冲突 → 以新为准，删掉旧的并留痕
                # （如新提取 dislikes["辣"]，就把旧 likes 里的"辣"移除，而不是跳过新值）
                if opposite and item in merged.get(opposite, []):
                    merged[opposite] = [x for x in merged.get(opposite, []) if x != item]
            ...
            if added:
                updates[key] = added
            if replaced:
                updates.setdefault("replaced", []).extend(replaced)
        # basic_info 合并：事实/偏好漂移以新值覆盖旧值，旧值留痕到历史
        new_basic = extracted.get("basic_info", {})
        if new_basic:
            hist = merged.setdefault("basic_info_history", {})
            for k, v in new_basic.items():
                ...
                if old_v == v:
                    continue
                if old_v:
                    hist.setdefault(k, []).append({"old": old_v, "at": time.time()})
