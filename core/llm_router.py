...
P4 序号7：实现真正的 SSE 流式输出，逐 token yield，支持打字机效果。
模型优先级：
  1级（主力）：豆包（火山方舟）
...
class BaseLLMClient:
    ...
    所有模型均兼容 OpenAI API 格式，因此非流式和流式调用都在基类统一实现。
    """
    name: str = "base"
    ...
    async def chat_stream(
        ...
        """
        流式聊天接口（SSE 格式，OpenAI 兼容 API 通用）。
        逐 token yield 模型输出，实现打字机效果。
        P4 序号7：真正的流式实现，不再降级为非流式。
        """
        if not self.available:
            logger.warning(f"[{self.name}] 不可用，跳过流式调用")
            return
        ...
        try:
            async with httpx.AsyncClient(timeout=timeout) as client:
                async with client.stream(
                    ...
                    if resp.status_code != 200:
                        ...
                        logger.error(
                            f"[LLM][{self.name}] 流式 HTTP{resp.status_code}: {error_text[:200]}"
                        )
                        return
            ...
            logger.info(
                f"[LLM][{self.name}] 流式成功 耗时={dur:.2f}s "
                f"输出长度={len(total_text)}"
            )
        except httpx.TimeoutException:
            logger.warning(f"[LLM][{self.name}] 流式超时 ({timeout}s)")
        except httpx.ConnectError as e:
            logger.warning(f"[LLM][{self.name}] 流式连接失败: {e}")
        except Exception as e:
            logger.warning(f"[LLM][{self.name}] 流式异常: {type(e).__name__}: {e}")
...
class LLMRouter:
    ...
    async def chat_stream(
        ...
        """
        智能降级流式聊天接口。自动选择可用模型，失败自动降级。
        P4 序号7：路由器级别的流式调用，支持降级。
        注意：流式调用中如果当前模型失败，已 yield 的内容不会回滚，
              会切换到下一个模型继续生成（可能导致内容不连贯，这是流式降级的固有问题）。
        Args:
            messages: 对话消息
            temperature: 温度
            max_tokens: 最大输出
        ...
        for _ in range(3):
            client = self._select_client()
            if client is None:
                logger.error("[LLMRouter] 所有级别都不可用，流式终止")
                return
            ...
            try:
                ...
                if got_any_content:
                    self._on_success(client)
                    return
                # 没有输出任何内容（可能是空回复或静默失败），尝试下一个模型
                logger.warning(f"[LLMRouter] {client.name} 流式无输出，尝试降级")
                self._on_fail(client)
            except Exception as e:
                logger.warning(f"[LLMRouter] {client.name} 流式异常: {type(e).__name__}: {e}")
                self._on_fail(client)
                # 如果已经输出了部分内容，就不降级了（避免内容混乱）
                if got_any_content:
                    logger.warning("[LLMRouter] 已输出部分内容，不降级，流式终止")
                    return
                # 还没输出内容，继续尝试下一个模型
                continue

        logger.error("[LLMRouter] 所有模型流式调用失败")
