    ...
    async def tick(self) -> Dict[str, Any]:
        ...
        try:
            ...
            for row in rows:
                ...
                # 先粗算一次（不含记忆分），用于决定是否值得查记忆
                prelim = self.engine.calc(intimacy, attachment, idle_hours, 0.0, mood, hour)
                # 分数太低且不可能靠记忆翻盘，跳过（省一次检索）
                if prelim["score"] < SCORE_THRESHOLD_MEMORY_BONUS - 15:
                    continue

                # 检索相关记忆
                related = None
                if prelim["score"] >= 30 or idle_hours >= 24:
                    related = await fetch_related_memory(user_id, role_id)
                mem_weight = 0.0
                if related:
                    try:
                        mem_weight = float(related.get("importance", 60))
                    except (TypeError, ValueError):
                        mem_weight = 60.0

                motivation = self.engine.calc(intimacy, attachment, idle_hours, mem_weight, mood, hour)
                score = motivation["score"]
                has_mem_event = related is not None and mem_weight >= 60

                decision = self.policy.check(conn, user_id, role_id, intimacy, score, has_mem_event)
                if not decision["allowed"]:
                    continue

                # 取角色名（从人格后端拉，失败则用 role_id）
                ...
                content = await generate_proactive_message(
                    ...
                    related, idle_hours, intimacy, mood)
                if not content:
                    continue
                ...
                logger.info(f"主动消息: user={user_id} role={role_id} score={score} "
                            f"reason={event['reason_type']} delivered={delivered} content={content[:30]}")
                # 每条之间稍微间隔，避免瞬间打爆
...
async def lifespan(app: FastAPI):
    init_db()
    logger.info(f"💌 主动消息后端 v10.0 启动 - 端口 {PORT}")
    scheduler.start()
    yield
...
app = FastAPI(title="Proactive Server", version="10.0.0", lifespan=lifespan)
